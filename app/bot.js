...
bot.on('channel_post', (ctx, next) => {
  let sql = `INSERT INTO messages (message, title, content, source, link) VALUES (?, ?, ?, ?, ?)`;

  if (ctx.chat.id == process.env.GROUP_ID) {
    // Escape all tags first
    let post = createPost(ctx.channelPost);

    database.run(sql, [post.message, post.title, post.content, post.source, post.link], (err) => {
      if (err) {
        return console.error(err.message);
      }
    });
...
bot.on('edited_channel_post', (ctx, next) => {
  ...
  if (ctx.chat.id == process.env.GROUP_ID) {
    ...
    database.run(sql, [post.title, post.content, post.source, post.link, post.message], (err) => {
      if (err) {
        return console.error(err.message);
      }
    });
...
bot.on('forward', (ctx) => {
  ...
  if (ctx.message.forward_from_chat.id == process.env.GROUP_ID) {
    ...
    database.run(sql, [msg.forward_from_message_id], (err) => {
      if (err) {
        return console.error(err.message);
      }
